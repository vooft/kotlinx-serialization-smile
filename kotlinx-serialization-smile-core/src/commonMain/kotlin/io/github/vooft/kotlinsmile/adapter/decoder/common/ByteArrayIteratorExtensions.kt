...
fun ByteArrayIterator.peekValueToken(): SmileValueToken {
    val byte = next()
    rollback(1)
    return SmileToken.valueToken(byte) ?: error("Unknown key token: 0x${byte.toUByte().toString(16)}")
}

fun ByteArrayIterator.peekKeyToken(): SmileKeyToken {
    val byte = next()
    rollback(1)
    return SmileToken.keyToken(byte) ?: error("Unknown key token: 0x${byte.toUByte().toString(16)}")
}
